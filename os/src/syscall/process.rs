...
pub fn sys_trace(trace_request: usize, id: usize, data: usize) -> isize {
    ...
    match trace_request {
        0 => {
            ...
            trace!("kernel: sys_trace: read byte at address {:#x} = {}", id, value);
            return value as isize;
        }
        1 => {
            unsafe { *(id as *mut u8) = data as u8 };
            trace!("kernel: sys_trace: write byte {} to address {:#x}", data, id);
            return 0;
        }
        2 => {
            let count = get_syscall_count(id);
            trace!("kernel: sys_trace: syscall {} called {} times", id, count);
            return count;
        }
        _ => {
            return -1;
